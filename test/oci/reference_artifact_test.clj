...
;; interpreter does?
;;
;; `reference_kotoba_test.clj` drives the guest through `kotoba.kir`, which is not what
...
(ns oci.reference-artifact-test
  ...
  (delay
    (let [{:keys [kotoba nbb runtime]} @tools]
      (if-not (and kotoba nbb runtime)
        ...
        (let [r (shell/sh nbb "test/oci/artifact_probe.cljs" kotoba amu-root)
...
(def ^:private kir
  (delay (:kir (compiler/compile-project
                {'oci.reference (slurp (io/file (System/getProperty "user.dir") "kotoba/oci/reference.kotoba"))}
...
(deftest the-native-backend-admits-this-guest
  ...
  (if-let [bin (:kotoba @tools)]
    (let [out (str (System/getProperty "java.io.tmpdir") "/oci-reference-native.kexe")
          r (shell/sh bin "-M" "compile"
                      (str (io/file (System/getProperty "user.dir") "kotoba/oci/reference.kotoba"))
